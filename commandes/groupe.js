...
zokou({ 
  ...
  if (superUser || verifAdmin) {
    const enetatoui = await verifierEtatJid(dest);
    try {
      // Display instructions if no arguments are passed
      if (!arg || !arg[0] || arg === ' ') {
        repondre("antisticker on to activate the anti-sticker feature\nantisticker off to deactivate the anti-sticker feature\nantisticker action/remove to directly remove the sticker without notice\nantisticker action/warn to give warnings\nantisticker action/delete to remove the sticker without any sanctions\n\nPlease note that by default, the anti-sticker feature is set to delete.");
        return;
      }

      if (arg[0] === 'on') {
        if (enetatoui) {
          repondre("The antisticker feature is already activated for this group.");
        } else {
          await ajouterOuMettreAJourJid(dest, "oui");
          repondre("The antisticker feature has been successfully activated.");
        }

      } else if (arg[0] === "off") {
        if (enetatoui) {
          await ajouterOuMettreAJourJid(dest, "non");
          repondre("The antisticker feature has been successfully deactivated.");
        } else {
        ...
        if (action === 'remove' || action === 'warn' || action === 'delete') {
          await mettreAJourAction(dest, action);
          repondre(`The antisticker action has been updated to ${action}`);
        } else {
          repondre("The only actions available are warn, remove, and delete.");
        }
...
// Event handler to detect stickers and take action
zokou.on('message', async (message) => {
  if (message.type === 'sticker') {
    const groupSettings = await getGroupSettings(message.chat);
    const action = groupSettings.antistickerAction || 'delete'; // Default action is to delete
    
    // Remove the sticker from the chat
    await zokou.deleteMessage(message.chat, message.id);
    
    // Take action based on group settings
    if (action === 'warn') {
      await zokou.sendMessage(message.chat, `⚠️ Warning: @${message.sender} Stickers are not allowed in this group.`);
    } else if (action === 'remove') {
      await zokou.sendMessage(message.chat, `🚫 The sticker sent by @${message.sender} has been removed without notice.`);
    } else if (action === 'delete') {
      await zokou.sendMessage(message.chat, `🚫 The sticker sent by @${message.sender} has been automatically deleted.`);
    }
  }
});
